...
for ep in range(episodes):
    ...
    while not done:
        ...
        if len(buffer) >= batch_size:
            ...
            with torch.no_grad():
                next_a = policy_net(s_next).argmax(1)
                q_next = target_net(s_next).gather(1, next_a.unsqueeze(1)).squeeze()
    ...
    # time.sleep(0.25) # 기기 냉각을 위해 딜레이 추가
env.close()

env = gym.make("CartPole-v1", render_mode = "human", max_episode_steps = 1500)
